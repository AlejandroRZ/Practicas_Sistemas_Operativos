...
//Importamos la clase mlfqs-calculations
  ...
  if (thread_mlfqs)
    {
      ...
      t->recent_cpu = t->recent_cpu + (1 << FRACT_BITS);
...
{ 
  //Usamos la funcion mlfqs_calculate_recent_cpu para calcular el valor de recent_cpu
...
thread_tick_one_second (void)
{
  ...
  load_avg = mlfqs_calculate_load_avg (load_avg, num_of_waiting_threads);
...
{
  //Usamos la funcion mlfqs_calculate_priority para calcular el valor de priority

  int new_priority = mlfqs_calculate_priority (t->recent_cpu, t->nice);
